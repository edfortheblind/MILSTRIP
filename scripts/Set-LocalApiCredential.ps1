param([ValidateSet('stage','prod')][string]$Environment = 'stage')
...
$credentialName = if ($Environment -eq 'prod') { 'api-prod-credential.json' } else { 'api-credential.json' }
$env:MILSTRIP_API_CREDENTIAL_FILE = Join-Path $privateDirectory $credentialName
